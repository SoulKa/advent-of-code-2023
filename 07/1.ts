...
export default async function run(input: string) {
  const plays = input.split("\n").map((line) => {
    ...
    return {
      play: getHighestPlay(hand),
      bid,
    };
  });

  // sort by highest kinds of plays (ascending)
  plays.sort((a, b) => a.play.kind - b.play.kind);
  ...
  switch (kind) {
    ...
    case Kind.FIVE_OF_A_KIND:
      for (const [range, secondary] of hand.getCuts(PAIR_SIZE_MAP[kind])) {
        if (allCardsEqual(range)) return [[range], secondary];
      }
      break;
    case Kind.TWO_PAIR:
      let pairs = [] as Range[];
      let cuts = hand.getCuts(2);
      for (let i = 0; i < cuts.length; i++) {
        const [range, remainder] = cuts[i];
        if (allCardsEqual(range)) {
          pairs.push(range);
          cuts = new Hand(remainder).getCuts(2);
        }
      }
      break;
    case Kind.FULL_HOUSE:
      ...
      if (allCardsEqual(highCut[0]) && allCardsEqual(highCut[1])) {
        return cutToPlay(highCut);
      } else if (allCardsEqual(lowCut[0]) && allCardsEqual(lowCut[1])) {
        return cutToPlay([lowCut[1], lowCut[0]]);
      }
      break;
    default:
...
function cutToPlay(range: Cut): Play {
